...
func (m *Model) toolbar(width int) (string, []hitRegion) {
	...
	switch m.page {
	case overview:
		wanted = []string{"mode", "tun", "targets", "tool-checks", "overview-inspect", "probe-ip", "probe-latency", "history-window", "graph-style"}
	case proxies:
	...
	if m.page == overview {
		...
		short["tun"] = "TUN: " + boolLabel(object(m.configData()["tun"]), "enable")
	}
	actions := m.actions()
	var parts []string
	var hits []hitRegion
