...
func applyManaged(ctx context.Context, t config.Target, p Plan, o Options) (Receipt, error) {
	...
	now := time.Now().UTC()
	r := Receipt{ID: hex.EncodeToString(id[:]), TargetID: t.ID, Binding: binding(t), Owner: managedrpi.Kind, File: managedrpi.Profile, Rule: p.Rule, Domain: p.Domain, Prefix: p.Prefix, Policy: p.Policy, Status: "prepared", CreatedAt: now, UpdatedAt: now, Digest: p.Digest, BeforeSHA256: p.Owner.file.SHA256, AfterSHA256: sha(p.after), BrokerReceipt: p.brokerReceipt}
	if r.BrokerReceipt == "" {
		return r, errors.New("managed RPi preview has no owner receipt")
	}
	if _, err := receiptDirectory(o, r.ID, true); err != nil {
		return r, err
	}
...
func managedReceipt(ctx context.Context, t config.Target, r Receipt, operation string, o Options) (Receipt, error) {
	...
	if err != nil {
		...
		r.Message = "RPi-ImmortalWrt 交易結果尚未確認；先 verify，勿自動重試。"
		_ = saveReceipt(o, r)
		return r, err
	}
	r.Status = out.State
