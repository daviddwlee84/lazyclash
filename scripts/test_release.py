...
class ReleaseTests(unittest.TestCase):
    ...
    def modern_fixture(self, source=True, evidence=False):
        ...
        for old in self.root.glob("*.tar.gz"):
            new = old.with_name(old.name.replace("0.1.0", "0.1.9"))
            old.rename(new)
            rows.append(f"{release.digest(new)}  {new.name}")
        if source:
            archive_path = self.root / "example_0.1.9_source.tar.gz"
    ...
    def test_new_source_asset_is_required_and_verified(self):
        self.modern_fixture()
        self.assertEqual(len(release.verify_dist(self.root, "example", "example", "0.1.9")), 6)
        (self.root / "example_0.1.9_source.tar.gz").write_bytes(b"corrupt")
        with self.assertRaisesRegex(ValueError, "checksum mismatch"):
            release.verify_dist(self.root, "example", "example", "0.1.9")
